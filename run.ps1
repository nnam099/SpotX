...
function CallLang($clg) {
    ...
    # [SECURITY FIX] Load language file from local disk instead of downloading
    # and executing remote code via Invoke-Expression.
    $localLangFile = Join-Path $PSScriptRoot "scripts\installer-lang\$clg.ps1"

    # Fallback to English if the requested language file doesn't exist locally
    if (-not (Test-Path -LiteralPath $localLangFile -PathType Leaf)) {
        $clg = 'en'
        $localLangFile = Join-Path $PSScriptRoot "scripts\installer-lang\en.ps1"
    }

    try {
        # Read the local file as UTF-8 to avoid encoding errors with special characters
        $scriptContent = Get-Content -LiteralPath $localLangFile -Raw -Encoding UTF8
        # Execute the script content securely without fetching from internet
        $scriptBlock = [scriptblock]::Create($scriptContent)
        return $scriptBlock.InvokeReturnAsIs()
    }
    catch {
        Write-Host "Error loading language file: $localLangFile"
...
# updated Russian translation — [SECURITY FIX] load from local file instead of remote URL
if ($langCode -eq 'ru' -and [version]$offline -ge [version]"1.1.92.644") {

    $localRuJson = Join-Path $PSScriptRoot "patches\Augmented translation\ru.json"

    if (Test-Path -LiteralPath $localRuJson -PathType Leaf) {
        try {
            $webjsonru = Get-Content -LiteralPath $localRuJson -Raw -Encoding UTF8 | ConvertFrom-Json
            if ($webjsonru -ne $null) {
                $ru = $true
            }
        }
        catch {
            Write-Warning "Failed to load local ru.json: $_"
        }
    }
}
